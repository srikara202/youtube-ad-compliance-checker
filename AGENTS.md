...
<!-- BEGIN CROSS-TOOL-SYNC (generated by sync-agents.ps1 - do not edit inside markers) -->
## Cross-tool project context (Claude Code / Codex / OpenCode)
- Shared memory store for this project: C:\Users\srika\.claude\projects\C--D-Drive-Python-Projects-youtube-ad-compliance-checker\memory
  Read C:\Users\srika\.claude\projects\C--D-Drive-Python-Projects-youtube-ad-compliance-checker\memory\MEMORY.md at session start (one line per memory); open a memory file only when its topic is relevant. When you learn a durable fact, preference, correction, or decision worth keeping: write/update a .md memory file there (frontmatter: name, description, metadata.type one of user|feedback|project|reference; for feedback/project add **Why:** and **How to apply:**) and add/refresh its one-line entry in MEMORY.md. Fix or delete stale memories instead of duplicating.
- Session handoff: if .claude\HANDOFF.md exists here, read it FIRST and continue from its "next step" - do not redo work marked done. Before wrapping up substantial work or switching tools (Claude Code / Codex / OpenCode), write/refresh it: goal, locked decisions, key files + state, verified vs unproven, open questions, exact next step.
<!-- END CROSS-TOOL-SYNC -->
